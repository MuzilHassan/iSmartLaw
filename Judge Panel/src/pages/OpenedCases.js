import React, { useState, useEffect } from "react";
import axios from "axios";
import AdminDashboard from "../components/AdminDashobard";
import Navbar from "../components/Navbar";
import { AuthContext } from "../context/authContext";
import { useContext } from "react";

function OpenedCases() {
  const [data, setData] = useState([]);
  const { user } = useContext(AuthContext);
  const [showModal, setShowModal] = useState(false);
  const [selectedCaseId, setSelectedCaseId] = useState(null);
  const [commentInput, setCommentInput] = useState("");

  useEffect(() => {
    fetchClosedCases();
  }, []);

  const fetchClosedCases = async () => {
    try {
      const response = await axios.get(`/judge/acceptedCases/${user._id}`);
      setData(response.data);
    } catch (error) {
      console.error("Error fetching closed cases:", error);
    }
  };

  const handleShowModal = (caseId) => {
    setShowModal(true);
    setSelectedCaseId(caseId);
  };

  const handleCloseModal = () => {
    setShowModal(false);
    setCommentInput("");
    setSelectedCaseId(null);
  };

  const handleAddComment = async () => {
    try {
      await axios.post(`/judge/addComment/${selectedCaseId}`, {
        comment: commentInput,
      });
      handleCloseModal();
      fetchClosedCases(); // Refresh the case data
    } catch (error) {
      console.error("Error adding comment:", error);
    }
  };

  return (
    <AdminDashboard>
      <Navbar />

      <div className="col-15 p-3 bg-light" style={{ height: "100vh" }}>
        <h3 style={{ fontWeight: "600" }}>Opened Cases</h3>
        <hr />
        <div className="container">
          <div className="row mb-4 mt-4">
            <div className="col-lg-12 col-sm-8">
              {data.length === 0 ? (
                <div className="jumbotron text-center">
                  <h1>No cases available</h1>
                  <p>There are no Opened cases at the moment.</p>
                </div>
              ) : (
                <div className="card" style={{ borderRadius: "10px" }}>
                  <div className="card-body">
                    <div className="card-title">
                      <h4 style={{ fontSize: "18px", fontWeight: "400" }}>
                        Opened Cases List
                      </h4>
                    </div>
                    <div className="table-responsive">
                      <table className="table table-hover">
                        <thead>
                          <tr>
                            <th>Case Number</th>
                            <th>Case Type</th>
                            <th>Case Description</th>
                            <th>Lawyer Name</th>
                            <th>Prosecution Name</th>
                            <th>Hearings Comments</th>
                            <th>Add Comments</th>
                          </tr>
                        </thead>
                        <tbody>
                          {data.map((item) => (
                            <tr key={item._id}>
                              <td>{item.caseNumber}</td>
                              <td>{item.caseType}</td>
                              <td>{item.caseDescription}</td>
                              <td>{item.lawyerId.name}</td>
                              <td>{item.clientId.name}</td>
                              <td>
                                {item.hearingComment.length > 0 ? (
                                  <ul>
                                    {item.hearingComment.map((comment, i) => (
                                      <li key={i}>{comment}</li>
                                    ))}
                                  </ul>
                                ) : (
                                  <p>No comments</p>
                                )}
                              </td>
                              <td>
                                <button
                                  className="btn btn-primary btn-sm"
                                  onClick={() => handleShowModal(item._id)}
                                >
                                  Add Comment
                                </button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                  </div>
                </div>
              )}
            </div>
          </div>
        </div>
      </div>

      {showModal && (
        <div
          className="modal d-flex align-items-center justify-content-center"
          tabIndex="-1"
          role="dialog"
          style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}
        >
          <div className="modal-dialog col-lg-12" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Add Comment</h5>
                <button
                  type="button"
                  className="close"
                  data-dismiss="modal"
                  aria-label="Close"
                  onClick={handleCloseModal}
                >
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div className="modal-body">
                <input
                  type="text"
                  placeholder="Enter your comment..."
                  style={{ width: "100%", border: "none", outline: "none" }}
                  value={commentInput}
                  onChange={(e) => setCommentInput(e.target.value)}
                />
              </div>
              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-primary"
                  onClick={handleAddComment}
                >
                  Add Comment
                </button>
                <button
                  type="button"
                  className="btn btn-secondary"
                  onClick={handleCloseModal}
                >
                  Close
                </button>
              </div>
            </div>
          </div>
        </div>
      )}
    </AdminDashboard>
  );
}

export default OpenedCases;
